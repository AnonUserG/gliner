...
Все параметры ниже задаются через `-e` (или через файл `.env`, см. `.env` в корне репозитория) при запуске контейнера — пересборка образа не требуется.

```bash
docker run -d -p 8000:8000 --name ner-gliner --env-file .env ner-gliner:1.0
```
...
Access-логи uvicorn для `/health` и `/metrics` отфильтрованы — это эндпоинты, которые периодически опрашивают Docker healthcheck и Prometheus, и без фильтра они забивали бы лог записями вида `"GET /metrics HTTP/1.1" 200 OK` каждые 15-30 секунд. Запросы к `/extract` и `/extract_batch` логируются как обычно.
